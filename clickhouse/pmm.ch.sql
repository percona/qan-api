CREATE TABLE IF NOT EXISTS agent_configs (
    agent_instance_id UInt32,
    service String,
    other_instance_id UInt16,
    in_file String,
    running String,
    updated DateTime
) ENGINE = Log(agent_instance_id, service, other_instance_id);
...
CREATE TABLE IF NOT EXISTS agent_log (
    ...
    service String,
    msg String
) ENGINE = Log(instance_id, sec, `level`);
...
CREATE TABLE IF NOT EXISTS instances ( 
    ...
    created DateTime,
    deleted DateTime
) ENGINE = Log(uuid, name, subsystem_id, deleted);
...
) ENGINE = Log(query_class_id, checksum); 
...
CREATE TABLE IF NOT EXISTS query_examples (
    ...
    Query_time Float32,
    query String
) ENGINE = Log(query_class_id, instance_id, period);
...
CREATE TABLE IF NOT EXISTS query_global_metrics ( 
    ...
    end_ts DateTime,
    run_time Float32,
    total_query_count UInt64,
    unique_query_count UInt64,
    rate_type String,
    rate_limit UInt8,
    log_file String,
    log_file_size UInt64,
    start_offset UInt64,
    end_offset UInt64,
    stop_offset UInt64,
    Query_time_sum Float32,
    ...
    No_index_used_sum UInt64,
    No_good_index_used_sum UInt64
) ENGINE = Log(instance_id, start_ts); 
